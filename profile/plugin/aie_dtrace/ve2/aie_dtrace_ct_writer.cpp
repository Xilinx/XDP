    ...
    const std::string& outputPath)
{
  ...
  const bool hasL2L2Counters = std::any_of(allCounters.begin(), allCounters.end(),
      [](const CTCounterInfo& ctr) { return ctr.metricSet == "input_ports"; });
  ...
  if (hasL2L2Counters)
    ctFile << "# Memtile L2-L2 inter-stamp halo counters when tile_based_memory_tile_metrics includes input_ports\n";
  ctFile << "\n";
    ...
    std::vector<CTRegisterWrite>& beginWrites)
{
  ...
  std::stringstream l2Msg;
  l2Msg << "AIE dtrace: Appending " << counterPoints.size()
        << " inter-stamp memtile L2-L2 counters to CT (start_col="
        << static_cast<int>(partitionStartCol) << ", num_cols=" << numCols << ")";
