...
namespace xdp {
                                             ...
                                             void* elf_handle)
  {
    ...
    // Register the per-run counter metadata in the process-lifetime registry so it can
    // be injected into the JSON dtrace dump on hw context teardown. This must not live
    // in the plugin/impl instance: the plugin's static instance can be destroyed before
    // the hw context writes the coalesced dump, so the injection is driven from
    // endAIEDtracePoll via aie::dtrace::injectPendingMetadata() instead.
    aie::dtrace::registerCounterMetadata(static_cast<uint32_t>(slotIdx), run_uid, metadataJson);
